...
SELECT admission_type_id
	, COALESCE(TRIM(admission_type_description), 'Not Selected') AS admission_type_description
...
SELECT discharge_disposition_id
	, COALESCE(TRIM(discharge_disposition_description), 'Not Selected') AS discharge_disposition_description
...
SELECT admission_source_id
	, COALESCE(TRIM(admission_source_description), 'Not Selected') AS admission_source_description
FROM bronze_admission_source;


-- Insert data from bronze_patient to silver_patient    
INSERT INTO silver_patient (patient_nbr, race, gender, age, weight)
SELECT patient_nbr
    , CASE
...
SELECT encounter_id,patient_nbr,admission_type_id,discharge_disposition_id,admission_source_id,time_in_hospital
    ...
    , COALESCE(max_glu_serum, 'None') AS max_glu_serum
